...
public class BoxesController : ControllerBase
{
    ...
    {
        var command = new UpdateBoxCommand(id, request.Name, request.ImageBase64, request.Items);
        var box = await _mediator.Send(command, cancellationToken);
        return Ok(box);
    }
...
public record UpdateBoxRequest(string? Name, string? ImageBase64, List<UpdateItemRequest>? Items);
